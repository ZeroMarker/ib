...
function App() {
  ...
  return user ? <Dashboard user={user} onLogout={() => setUser(null)} /> : <Auth onAuthenticated={setUser} />
}
...
function Auth({ onAuthenticated }: { onAuthenticated: (user: AuthUser) => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const user = await api<AuthUser>(`auth/${mode}`, json({ email, password }))
...
function Dashboard({ user, onLogout }: { user: AuthUser; onLogout: () => void }) {
  ...
  useEffect(() => { load() }, [])
  useEffect(() => { if (overview?.contracts.length && !overview.contracts.some((item) => String(item.conid) === orderForm.conid)) setOrderForm((form) => ({ ...form, conid: String(overview.contracts[0].conid) })) }, [overview])

  const symbols = useMemo(() => new Map((overview?.contracts ?? []).map((item) => [item.conid, item.symbol])), [overview])
  const action = async (work: () => Promise<void>, success: string) => { try { await work(); setNotice(success); setError(''); await load() } catch (reason) { setError(reason instanceof Error ? reason.message : '操作失败') } }
  const addContract = (event: React.FormEvent) => { event.preventDefault(); return action(async () => { await api('trading/contracts', json({ ...contractForm, conid: Number(contractForm.conid), sec_type: 'STK' })); setContractForm({ conid: '', symbol: '', exchange: 'SMART', currency: 'USD' }) }, '合约已添加。') }
  const placeOrder = (event: React.FormEvent) => { event.preventDefault(); return action(async () => { await api('trading/orders', json({ conid: Number(orderForm.conid), side: orderForm.side, order_type: orderForm.order_type, quantity: orderForm.quantity, lmt_price: orderForm.order_type === 'LMT' ? orderForm.price || null : null, aux_price: null })) }, '订单已提交。') }
  const setCash = (event: React.FormEvent) => { event.preventDefault(); return action(async () => { await api('trading/cash', json(cashForm)) }, '现金余额已更新。') }
  const cancel = (orderId: number) => action(async () => { await api(`trading/orders/${orderId}/cancel`, { method: 'POST' }) }, '订单已撤销。')
  const fill = (orderId: number) => { const price = window.prompt('请输入模拟成交价格'); if (price) return action(async () => { await api(`trading/orders/${orderId}/fill`, json({ price })) }, '模拟成交已记账。') }
  const logout = async () => { await api('auth/logout', { method: 'POST' }).catch(() => {}); onLogout() }

  return <main className="dashboard-page">
    <header className="dashboard-header"><div className="brand-line"><div className="brand-mark small">ib</div><div><p className="eyebrow">YOUR SIMULATION SPACE</p><h1>交易工作台</h1></div></div><div className="header-actions"><span className="user-pill"><span className="status-dot" />{user.email}</span><button className="text-button" onClick={logout}>退出登录</button></div></header>
    ...
    {!overview ? <div className="loading-card">正在读取账户数据…</div> : <>
      ...
      <DataTable title="订单" eyebrow="ORDERS" action={<button className="text-button" onClick={load}>刷新</button>}><table><thead><tr><th>订单</th><th>合约</th><th>方向</th><th>数量</th><th>状态</th><th /></tr></thead><tbody>{overview.orders.length ? overview.orders.map((order) => <tr key={`${order.account_id}-${order.order_id}`}><td>#{order.order_id}</td><td>{symbols.get(order.conid) ?? `#${order.conid}`}</td><td>{order.side}</td><td>{order.total_quantity}</td><td>{order.status}</td><td>{['Submitted', 'PreSubmitted'].includes(order.status) && <><button className="table-action" onClick={() => cancel(order.order_id)}>撤单</button><button className="table-action fill-action" onClick={() => fill(order.order_id)}>成交</button></>}</td></tr>) : <Empty colSpan={6} />}</tbody></table></DataTable>
      <div className="data-columns"><DataTable title="持仓" eyebrow="POSITIONS"><table><thead><tr><th>合约</th><th>数量</th><th>平均成本</th></tr></thead><tbody>{overview.positions.length ? overview.positions.map((item) => <tr key={item.conid}><td>{symbols.get(item.conid) ?? `#${item.conid}`}</td><td>{item.position}</td><td>{item.avg_cost ?? '—'}</td></tr>) : <Empty colSpan={3} />}</tbody></table></DataTable><DataTable title="成交" eyebrow="FILLS"><table><thead><tr><th>执行 ID</th><th>订单</th><th>数量</th><th>价格</th></tr></thead><tbody>{overview.fills.length ? overview.fills.map((item) => <tr key={item.exec_id}><td>{item.exec_id}</td><td>#{item.order_id}</td><td>{item.quantity}</td><td>{item.price}</td></tr>) : <Empty colSpan={4} />}</tbody></table></DataTable></div>
    </>}
...
function DataTable({ eyebrow, title, action, children }: { eyebrow: string; title: string; action?: React.ReactNode; children: React.ReactNode }) { return <section className="data-panel"><div className="panel-heading"><PanelTitle eyebrow={eyebrow} title={title} />{action}</div><div className="table-wrap">{children}</div></section> }
